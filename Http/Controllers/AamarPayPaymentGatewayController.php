...
class AamarPayPaymentGatewayController extends Controller
{
    /**
    ...
    private function chargeCustomerForLandlordPricePlanPurchase($args){
        ...
        $store_id = get_static_option("aamar_pay_store_id");
        $signature_key = get_static_option("aamar_pay_signature_key");
        ...
        $callback_url = route('aamarpaypaymentgateway.tenant.price.plan.ipn');
        if ($args["payment_for"] === "landlord"){
            $callback_url = route('aamarpaypaymentgateway.landlord.price.plan.ipn');
        }
